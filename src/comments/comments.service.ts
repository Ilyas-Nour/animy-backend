...
export class CommentsService {
    ...
    async create(userId: string, data: { providerId: string, content: string, parentId?: string }) {
        try {
            ...
            const comment = await this.prisma.comment.create({
                ...
                include: {
                    user: { select: { id: true, username: true, avatar: true, firstName: true } }
                }
            });
            ...
            throw new InternalServerErrorException(`Comment failed: ${error.message}`);
        }
    }
    ...
    async update(id: string, userId: string, content: string) {
        ...
        return this.prisma.comment.update({
            where: { id },
            data: { content },
            include: { user: { select: { id: true, username: true, avatar: true, firstName: true } } }
        });
    }
    ...
    async findAll(providerId: string, currentUserId?: string) {
        ...
        return comments.map(comment => ({
            ...comment,
            isLiked: currentUserId ? comment.reactions.some(r => r.userId === currentUserId && r.type === 'LIKE') : false,
            replies: comment.replies.map(reply => ({
                ...reply,
                isLiked: currentUserId ? reply.reactions.some(r => r.userId === currentUserId && r.type === 'LIKE') : false,
            }))
        }));
    }
}
