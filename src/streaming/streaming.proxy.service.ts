...
export class StreamingProxyService {
    ...
    async proxy(url: string, referer: string, res: Response) {
        try {
            ...
            if (contentType.includes('mpegurl') || contentType.includes('application/vnd.apple.mpegurl') || url.includes('.m3u8')) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    response.data.on('end', () => {
                        const rewrittenManifest = this.rewriteManifest(manifestData, url, referer);
            ...
            throw new HttpException('Failed to proxy video source', HttpStatus.BAD_GATEWAY);
        }
    }
     ...
     */
    private rewriteManifest(content: string, originalUrl: string, referer: string): string {
