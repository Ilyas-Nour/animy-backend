    ...
    async toggleReaction(userId: string, data: {
        ...
        try {
            ...
            if (data.providerId) {
                ...
                let engagement = await this.prisma.newsEngagement.findUnique({ where: { providerId: data.providerId } });
                if (!engagement) {
            ...
            if (data.commentId) {
                const comment = await this.prisma.comment.findUnique({ where: { id: data.commentId } });
            ...
            throw new InternalServerErrorException(`Reaction failed: ${error.message}`);
        }
    }
}
